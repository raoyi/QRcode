...
if os.path.exists(bompath):
    with open(bompath,'r',encoding='utf-8') as file:
        ...
        for line in file.readlines():
            ...
            #判断IR camera
            if 'ircamkey' in valist:
                if ircamkey+'=' in line:
                    ircam = line.split('=')[1].replace('\n','').strip()
                    if ircam == 'IRCAM YES':
                        replace_dict['ircamkey'] = 'Y'
                    else:
                        replace_dict['ircamkey'] = 'N'

            # 判断是否存在A壳Y灯，copy自PU1230302
            if 'ylogopn' in valist:
                if 'MTNAMEPN=' in line:
