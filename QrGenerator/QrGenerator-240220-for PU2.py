#!/usr/bin/env python
# -*- coding: utf-8 -*-
# 基于 230210-for PU2 版本修改
# 1. KBSPEC2由RGB BL改为BL
# 2. - jpkbkey：是否日本键盘，英文逗号分隔，不包含空格，第一个值为BOM中的键名，后面列出应该显示Y的所有值
# ini 例子： jpkbkey = KBSPEC3,JA,MATRIX-JP
import os
import qrcode
...
if os.path.exists(bompath):
    with open(bompath,'r',encoding='utf-8') as file:
        ...
        for line in file.readlines():
            ...
            # for pu2
            # 判断键盘背光
            if 'kbbglkey' in valist:
                if 'KBPN=' in line:
                    kbpn = line.split('=')[1].replace('\n','').strip()
                    kbbglflag = kbpn[-3]
                    if kbbglval.count(kbbglflag) != 0:
                        replace_dict['kbbglkey'] = 'Y'
                    else:
                        replace_dict['kbbglkey'] = 'N'

            # for pu2
            # JPKB key
            if 'jpkbkey' in valist:
                if jpkbkey.split(',')[0]+'=' in line:
                    kbtype = line.split('=')[1].replace('\n','').strip()
                    if kbtype in jpkbkey.split(',')[1:]:
                        replace_dict['jpkbkey'] = 'Y'
                    else:
                        replace_dict['jpkbkey'] = 'N'
